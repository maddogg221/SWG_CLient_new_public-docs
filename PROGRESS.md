...
- **Locomotion polish.** The core animation pipeline now works, including a real fix to how the game's own situation-to-clip selection logic resolves (see the technical notes). Walking specifically was narrowed down, via direct side-by-side comparison against the original game, to a stride-length difference — and every concrete hypothesis for a bug in this project's own code has since been tested and closed out clean, including cross-validation against an entirely independent, unrelated implementation. The recorded animation data, played back and combined correctly, appears to genuinely contain this range of motion — very likely not a defect in this project's own reimplementation. Attention has moved to the resting stance instead, which has been directly confirmed to differ from the original game's (arms held away from the body rather than resting at the sides). Two further candidate explanations have since been tested and also closed out clean — one correction value that looked like it might matter specifically for the arms turned out negligible once actually measured, and the several plausible ways a bone's rotation components could combine were swept and compared side by side, all landing on essentially the same pose. Along the way, a wrong assumption behind both tests was caught and corrected: "resting" turns out not to be a literal unanimated pose at all — the character is always playing a real, subtle idle animation, even standing still — which reframes the open question going forward. See the technical notes for the full account.
