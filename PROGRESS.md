...
- **Locomotion polish.** The core animation pipeline now works, including a real fix to how the game's own situation-to-clip selection logic resolves (see the technical notes). Walking specifically still has a known, unresolved visual defect under active investigation — see the technical notes for the numeric-diagnostic approach being used to track it down — along with confirming the character's resting stance matches what a player actually sees in the live game rather than an internal reference pose.
