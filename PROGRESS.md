...
- **Locomotion polish.** The core animation pipeline now works, including a real fix to how the game's own situation-to-clip selection logic resolves (see the technical notes). Walking specifically has been narrowed down, via direct side-by-side comparison against the original game, to a stride-length problem rather than the joint-direction bug it first appeared to be — not yet fixed. The resting stance has also now been directly confirmed to differ from the original game's (arms held away from the body rather than resting at the sides) — see the technical notes for both.
