...
## Milestone: real terrain grading, and real portal-based room visibility

Two long-standing known gaps closed in the same pass.

The first: the ground around a player-placed structure's own foundation had never matched the shaping a real placed building applies, leaving a visible seam right at a building's outer edge. Chasing an actual fix turned up a real surprise once the server-side placement logic was read directly: the live game applies essentially no terrain grading to ordinary player housing at all, on any size, on any planet — that grading mechanism is reserved for a small set of non-player structure categories. The seam this project had been trying to smooth away is one the real game itself simply accepts. This project's own terrain-grading feature is now scoped to match that real behavior exactly, rather than continuing to chase a "fix" for behavior the original game never actually has.

Getting to that correct conclusion took two wrong turns first, each caught by testing live rather than trusted from a clean-looking implementation: an early filename-matching heuristic picked a real, validly-structured data file that turned out to carry no actual height data at all (a decorative-only file, confirmed by reading its raw contents rather than assuming from its name), and a second attempt picked a real file sized for an ordinary small house, leaving a much larger structure's footprint mostly uncovered. Both were superseded once a real, authoritative mapping was found in the server's own placement scripts instead of continuing to guess at a filename pattern.

The second: standing inside a building, a neighboring room visible through an open doorway had never actually rendered until the player physically walked into it — an old, coarse "current room only" rule. Replaced with a real visibility test: each room's actual portal connections are walked against the camera's own view frustum, so an adjacent room now appears exactly when it's genuinely in view and disappears again once it isn't, matching how the real game's own interiors read. One implementation bug was caught before ever reaching a live test at all — an early version of the camera-frustum math had a subtle but complete inversion that would have silently rejected a point sitting in plain view — because the automated test suite exercising this new code was written before ever pointing a real camera at a real building, and it failed loudly and immediately.

A related collision-height problem was found live while testing the terrain-grading fix above, and remains open: a real building's entrance ramp still lets the character fall through the ground right at its outer edge. Several fix attempts each measurably narrowed the problem without fully closing it — this one is still being worked.
