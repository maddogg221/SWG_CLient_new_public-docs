...
- A real rendering pipeline (**Vulkan**), decided on and built after direct hands-on rendering experience with an earlier prototype backend. Live-verified: a real 3D view showing tracked objects, a minimap, and floating name labels, standing directly on real rendered terrain (an early placeholder ground grid was removed once real terrain covered everywhere it used to stand in for).
...
- **Real building rendering, inside and out** — player-placed structures (houses, guild halls) now render their full real geometry, both exterior and every real interior room, resolved from a format entirely distinct from the one ordinary objects use. Live-verified against a real player-owned house: a genuine three-story building with a working elevator, all real interior rooms rendering correctly.
- **Real skeletal animation** — characters and creatures now play back real keyframe animation (walking, idle motion) driven by the game's own animation-state and clip data, not a static pose. One specific, narrow, and unusually stubborn defect remains open in the resting stance specifically — see below and the technical notes.
...
- **An open call for help:** the character's resting stance holds its arms out and away from the body instead of at its sides, unlike the original game. This has been investigated more thoroughly than anything else in this project's history — every per-clip animation data source both relevant file formats involved can contain has now been read, checked against the platform's own original production code, and where a real gap existed, fixed (see the technical notes for the full account) — and none of it explains the difference. If you have real hands-on knowledge of this platform's animation pipeline or rest-pose conventions and recognize this symptom, please open an issue. Genuinely welcome, and would be credited.
- Movement is open-world walking only — no interiors, running, or other locomotion states yet. A building's real interior now renders, but the player's own position tracking has no concept yet of standing inside a room, so there's no floor underfoot once you step through a door.
